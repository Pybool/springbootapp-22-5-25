...
@Component({
  selector: 'app-two-fa-auth-verification',
  standalone: true,
  imports: [
    CommonModule,
    HttpClientModule,
...
export class TwoFaAuthVerificationComponent {
  data = '';
  email: any = null;
  ...
  ngOnInit() {
    this.email = window.localStorage.getItem("register-verify")
  }
  ...
  onOtpSuccess(response: any) {
    if (response.status) {
      ...
      this.snackbar
        .show(response.message || 'Registration was successfull')
        .then(() => {
          this.router.navigate(['/signin']); // Default home page
        });
    } else {
      this.isFullPageSpinnerLoading = false;
      this.snackbar.show(
